...
function s.initial_effect(c)
	-- Add Chaos Ritual Monster to hand
	...
	-- Banish to reveal and shuffle + draw
...
function s.thfilter(c)
	return c:IsType(TYPE_RITUAL) and c:IsType(TYPE_MONSTER) and c:IsRace(RACE_SPELLCASTER) 
		and c:IsSetCard(0xcf) and c:IsAbleToHand()
end
...
function s.banishthtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then 
		return Duel.IsExistingMatchingCard(s.banishthfilter,tp,LOCATION_HAND,0,1,nil)
			and Duel.IsExistingMatchingCard(s.todeckfilter,tp,LOCATION_GRAVE+LOCATION_REMOVED,0,1,nil)
	end
...
function s.banishthop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #rg>0 then
		...
		if #g>0 then
			...
			Duel.ShuffleDeck(tp) -- 🔵 Aquí barajamos
			Duel.BreakEffect() -- 🔵 Separa las acciones, luego robamos
