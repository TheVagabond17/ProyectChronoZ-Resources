...
-- Check if Special Summoned by "Koa'ki Meiru" card effect
function s.spcon(e,tp,eg,ep,ev,re,r,rp)
	return re and re:GetHandler():IsSetCard(0x1d) and re:GetHandler():IsType(TYPE_MONSTER)
end

function s.tgfilter(c)
	return c:IsSetCard(0x1d) and c:IsAbleToGrave()
end

function s.spfilter(c,e,tp)
	return c:IsSetCard(0x1d) and c:IsLevelBelow(4) and not c:IsCode(id) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if Duel.Release(c,REASON_COST)==0 then return end -- Se tributa primero, liberando espacio
	...
	if #tg>0 and Duel.SendtoGrave(tg,REASON_EFFECT)>0 then
		-- Ahora sí se verifica el espacio en el campo después de la tributa
